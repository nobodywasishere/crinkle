...
| Template parser strictness (unknown tags, mismatched end-tags) | in progress (strict mode for unknown/end-tag structure + improved spans/messages) | `spec/parser_spec.cr` strict-mode coverage, `fixtures/parser/unexpected_end_tag.*` |
...
- [ ] Implement template parser parity updates in `src/parser/parser.cr` (strict unknown-tag + strict tag-structure modes landed; remaining mismatch/unknown-tag parity defaults pending).
