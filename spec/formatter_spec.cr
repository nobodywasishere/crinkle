...
describe Crinkle::Formatter do
  describe "idempotency" do
    ...
    fixture_templates.each do |info|
      it "is idempotent for #{info.name}" do
        ...
        formatter1 = Crinkle::Formatter.new(source)
        output1 = formatter1.format

        formatter2 = Crinkle::Formatter.new(output1)
  ...
  describe "options" do
    it "respects space_inside_braces option" do
      ...
      with_spaces = Crinkle::Formatter.new(source, Crinkle::Formatter::Options.new(space_inside_braces: true))
      with_spaces.format.strip.should eq("{{ name }}")

      without_spaces = Crinkle::Formatter.new(source, Crinkle::Formatter::Options.new(space_inside_braces: false))
    ...
    it "respects space_around_operators option" do
      ...
      with_spaces = Crinkle::Formatter.new(source, Crinkle::Formatter::Options.new(space_around_operators: true))
      with_spaces.format.strip.should eq("{{ a + b }}")

      without_spaces = Crinkle::Formatter.new(source, Crinkle::Formatter::Options.new(space_around_operators: false))
